...
response = requests.get(api_url, headers={'X-Api-Key': 'iXD4zDXZ+CHNDZ6VqyhfdQ==R6A7Lu3V0k0S5ezY'})
...
if uploaded_top_view and uploaded_side_view:
    ...
    aruco_dict = cv2.aruco.Dictionary_get(cv2.aruco.DICT_5X5_50) if hasattr(cv2.aruco, 'Dictionary_get') else cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_5X5_50)
    ...
    if corners:
        ...
        mask_area = 100
        contours = detector.detect_objects(top_view_cv_image,int_corners)
        ...
        side_view_contours = detector.detect_objects(side_view_cv_image,int_corners)
